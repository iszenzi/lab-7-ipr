...
export async function ApiCheckAuth() {
    try {
        ...
        if (contentType && contentType.includes("application/json")) {
            ...
            return { error: true, detail: "шибка сервера (500). роверьте логи бекенда." };
        }
        if (DEBUG) console.debug("Authentication (access). Data received:", data1);

        if (data1.detail && data1.detail.includes("РўРѕРєРµРЅ РЅРµРґРµР№СЃС‚РІРёС‚РµР»РµРЅ")) {
...
export async function ApiDeleteUser() {
    try {
        ...
        if (data.detail && data.detail.includes("РўРѕРєРµРЅ РЅРµРґРµР№СЃС‚РІРёС‚РµР»РµРЅ")) {
